...
public class SimpleSerializable implements Cloneable {
	...
	@J2SIgnore
	protected static class ItemResult {
		
		public static ItemResult MISSING_DATA = new ItemResult(SIMPLE_MISSING_DATA);
		public static ItemResult INVALID_DATA = new ItemResult(SIMPLE_INVALID_DATA);
		public static ItemResult UNKNOWN_ERROR = new ItemResult(SIMPLE_UNKNOWN_ERROR);
		public static ItemResult ARRAY_TOO_LARGE = new ItemResult(SIMPLE_ARRAY_TOO_LARGE);
		
		public Object object;
		public int index;
		public int code;
		
		public ItemResult(Object object, int index) {
			this.object = object;
			this.index = index;
			this.code = SIMPLE_OK;
		}
...
return strBuf;
			...
			for (Iterator<Entry<String, Field>> itr = fields.entrySet().iterator(); itr.hasNext();) {
				...
				if (type == String.class) {
					...
					if (e == null) {
						...
						String value = String.valueOf(e.name());
						builder.append((char) (baseChar + value.length()));
						builder.append(value);
					}
				} else if (type == float.class) {
			...
			for (Iterator<Entry<String, Field>> itr = fields.entrySet().iterator(); itr.hasNext();) {
				...
				if (type == String.class) {
					...
					if (type == byte[].class) {
						...
						Enum<?>[] ss = (Enum<?>[]) field.get(this);
						if (ss == null && ignoring) continue;
						dos.writeByte((char)(baseChar + name.length()));
						dos.writeBytes(name);
						dos.writeBytes("AE");
						if (ss == null) {
							dos.writeByte('A'); // (char) (baseChar - 1));
							...
							for (int j = 0; j < ss.length; j++) {
								if (ss[j] == null) {
									...
									String value = ss[j].name();
									dos.writeByte((char) (baseChar + value.length()));
									dos.writeBytes(value);
								}
							}
						}
	...
	@J2SIgnore
	private void serializeArrayItem(StringBuilder builder, Class<?> type, Object target,
			List<SimpleSerializable> ssObjs, boolean supportsCompactBytes) throws Exception {
		char baseChar = 'B';
		if (type == String.class) {
	...
	@J2SIgnore
	private void serializeBytesArrayItem(DataOutputStream dos, Class<?> type, Object target,
			List<SimpleSerializable> ssObjs, boolean supportsCompactBytes) throws Exception {
		char baseChar = 'B';
		if (type == String.class) {
			String s = (String) target;
	...
	@J2SIgnore
	private int serializeLength(StringBuilder builder, int length) {
		char baseChar = 'B';
		if (length > 52) {
	...
	@J2SIgnore
	private int serializeBytesLength(DataOutputStream dos, int length) throws IOException {
		char baseChar = 'B';
		if (length > 52) {
			if (length > 0x1000000) { // 16 * 1024 * 1024
				return SIMPLE_ARRAY_TOO_LARGE;
...
if (ss != null) {
	 ...
	 */
	@J2SKeep
	private void serializeObject(StringBuilder builder, SimpleSerializable ss,
...
//						}
						...
						for (int i = 0; i < xs.length; i++) {
							String str = xs[i] != null ? xs[i].name() : null;
							if (withFormats) {
								builder.append(prefix);
								builder.append('\t');
							}
...
return true;
	...
	public boolean deserialize(String str) {
		...
		ssObjs.add(this);
		return deserialize(str, 0, ssObjs) == SIMPLE_OK;
	}
	...
	public boolean deserializeBytes(byte[] bytes) {
		List<SimpleSerializable> ssObjs = new LinkedList<SimpleSerializable>();
		ssObjs.add(this);
		return deserializeBytes(bytes, 0, ssObjs) == SIMPLE_OK;
	}
	...
	public boolean deserialize(String str, int start) {
		List<SimpleSerializable> ssObjs = new LinkedList<SimpleSerializable>();
		ssObjs.add(this);
		return deserialize(str, start, ssObjs) == SIMPLE_OK;
	}
	...
	public boolean deserializeBytes(byte[] bytes, int start) {
		List<SimpleSerializable> ssObjs = new LinkedList<SimpleSerializable>();
		ssObjs.add(this);
		return deserializeBytes(bytes, start, ssObjs) == SIMPLE_OK;
	}
				...
				try {
					if (l2 < 0 && l2 != -2) {
						...
						if (c2 == 'Z' || c2 == 'Y' || c2 == 'Q') {
							...
							for (int i = 0; i < l2; i++) {
								ItemResult o = deserializeArrayItem(str, index, end, ssObjs, itemType);
							...
							for (int i = 0; i < l2 / 2; i++) {
								ItemResult key = deserializeArrayItem(str, index, end, ssObjs, keyType);
								if (key.code != SIMPLE_OK) return key.code;
								index = key.index;
								ItemResult value = deserializeArrayItem(str, index, end, ssObjs, valueType);
						...
						case 'E': {
							Class<?> enumType = field.getType().getComponentType();
							Enum<?>[] enums = (Enum<?>[]) Array.newInstance(enumType, l2);
							for (int i = 0; i < l2; i++) {
								if (ss[i] == null || ss[i].length() == 0) continue;
								Enum<?>[] enumConstants = (Enum<?> []) enumType.getEnumConstants();
								for (int j = 0; j < enumConstants.length; j++) {
									if (enumConstants[j].name().equals(ss[i])) {
										enums[i] = enumConstants[j];
										break;
									}
								}
							}
							field.set(this, enums);
							break;
						}
						}
					}
					...
					case 'E':
						if (s != null && s.length() > 0) {
							...
							for (int i = 0; i < enumConstants.length; i++) {
								if (enumConstants[i].name().equals(s)) {
									eo = enumConstants[i];
									break;
								}
							}
	...
	public int deserializeBytes(byte[] bytes, int start, List<SimpleSerializable> ssObjs) {
		...
		int length = end - start;
		if (length <= 7) return SIMPLE_MISSING_DATA;
		if ('W' != bytes[start] || 'L' != bytes[start + 1] || 'L' != bytes[start + 2]) return SIMPLE_INVALID_DATA;
...
//					}
				...
				try {
					if (l2 < 0 && l2 != -2) {
						...
						if (c2 == 'Z' || c2 == 'Y' || c2 == 'Q') {
							...
							for (int i = 0; i < l2; i++) {
								ItemResult o = deserializeBytesArrayItem(bytes, index, end, ssObjs, itemType);
								if (o.code != SIMPLE_OK) return o.code;
								objCollection.add(o.object);
								index = o.index;
							}
							...
							Map<Object, Object> objMap = new HashMap<Object, Object>(l2);
							for (int i = 0; i < l2 / 2; i++) {
								ItemResult key = deserializeBytesArrayItem(bytes, index, end, ssObjs, keyType);
								if (key.code != SIMPLE_OK) return key.code;
								index = key.index;
								ItemResult value = deserializeBytesArrayItem(bytes, index, end, ssObjs, valueType);
								if (value.code != SIMPLE_OK) return value.code;
								index = value.index;
								objMap.put(key.object, value.object);
							}
	...
	private Class<?> exactGenericClass(Type extraGenericType) {
		...
		if (extraGenericType instanceof WildcardType) {
			WildcardType wt = (WildcardType) extraGenericType;
			Type extraType = null;
			for (Type upperType : wt.getUpperBounds()) {
				extraType = upperType;
				break;
			}
			if (extraType == null) {
				for (Type lowerType : wt.getLowerBounds()) {
					extraType = lowerType;
					break;
				}
			}
			...
			extraGenericType = extraType;
		}
		if (extraGenericType instanceof ParameterizedType) {
			ParameterizedType pt = (ParameterizedType) extraGenericType;
			extraGenericType = pt.getRawType();
		}
		
		String extraType = extraGenericType.toString();
		if (extraType.startsWith("class ")) {
			extraType = extraType.substring(6);
		}
		try {
			return Class.forName(extraType);
		} catch (Exception e) {
		}
		return null;
	}
	...
	protected ItemResult deserializeArrayItem(String str, int index, int end, List<SimpleSerializable> ssObjs, Type extraType) {
		...
		if (c2 == 'A' || c2 == 'Z' || c2 == 'Y') {
			...
			try {
				if (l2 < 0 && l2 != -2) {
					...
					if (c2 == '8') { // byte[]
						...
						index += l2;
						byte[] bs = byteStr.getBytes(ISO_8859_1);
						return new ItemResult(bs, index);
					}
					if (c2 == 'Z' || c2 == 'Y' || c2 == 'Q') {
						Collection<Object> objCollection = null;
						if (c2 == 'Z') {
						...
						Type itemType = null;
						if (extraType != null && extraType instanceof ParameterizedType) {
							ParameterizedType pt = (ParameterizedType) extraType;
							Type[] argTypes = pt.getActualTypeArguments();
							if (argTypes != null && argTypes.length > 0) {
								itemType = argTypes[0];
							}
						...
						return new ItemResult(objCollection, index);
					} else if (c2 == 'M') {
						Type keyType = null;
						Type valueType = null;
						if (extraType != null && extraType instanceof ParameterizedType) {
						...
						return new ItemResult(objMap, index);
					}
					String[] ss = new String[l2];
					for (int i = 0; i < l2; i++) {
						char c4 = str.charAt(index++);
					...
					case 'I': {
						...
						return new ItemResult(ns, index);
					}
					case 'X':
						return new ItemResult(ss, index);
					case 'O': {
						...
						return new ItemResult(sss, index);
					}
					case 'L': {
						long[] ls = new long[l2];
						for (int i = 0; i < l2; i++) {
						...
						return new ItemResult(ls, index);
					}
					case 'b': {
						boolean[] bs = new boolean[l2];
						for (int i = 0; i < l2; i++) {
					...
					case 'F': {
						...
						return new ItemResult(fs, index);
					}
					case 'D': {
						double[] ds = new double[l2];
						for (int i = 0; i < l2; i++) {
						...
						return new ItemResult(ds, index);
					}
					case 'S': {
						short[] sts = new short[l2];
						for (int i = 0; i < l2; i++) {
						...
						return new ItemResult(sts, index);
					}
					case 'B': {
						byte[] bs = new byte[l2];
						for (int i = 0; i < l2; i++) {
					...
					case 'C': {
						...
						return new ItemResult(cs, index);
					}
					case 'E': {
						Type compType = null;
						if (extraType instanceof GenericArrayType) {
							compType = ((GenericArrayType) extraType).getGenericComponentType();
						}
						Class<?>enumType = exactGenericClass(compType);
						if (enumType == null) return new ItemResult(null, index);
						Enum<?>[] enums = (Enum<?>[]) Array.newInstance(enumType, l2);
						for (int i = 0; i < l2; i++) {
							if (ss[i] == null || ss[i].length() == 0) continue;
							Enum<?>[] enumConstants = (Enum<?> []) enumType.getEnumConstants();
						...
						return new ItemResult(enums, index);
					}
					default :
						return new ItemResult(null, index);
					}
			...
			try {
				switch (c2) {
				case 'I':
					return new ItemResult(Integer.valueOf(s), index);
				case 's':
					return new ItemResult(s, index);
				case 'L':
					return new ItemResult(Long.valueOf(s), index);
				case 'b': {
					char c = s.charAt(0);
					return new ItemResult(Boolean.valueOf(c == '1' || c == 't'), index);
				}
				case 'O': 
					if (s != null) {
						SimpleSerializable ss = SimpleSerializable.parseInstance(s);
				...
				case 'E':
					if (s != null && s.length() > 0) {
						Class<?>extraClass = exactGenericClass(extraType);
						if (extraClass == null) return new ItemResult(null, index);
						Object eo = null;
						Enum<?>[] enumConstants = (Enum<?> []) extraClass.getEnumConstants();
						for (int i = 0; i < enumConstants.length; i++) {
							if (enumConstants[i].name().equals(s)) {
								eo = enumConstants[i];
								break;
						...
						return new ItemResult(eo, index);
					} else {
						return new ItemResult(null, index);
					}
				case 'F':
					return new ItemResult(Float.valueOf(s), index);
				case 'D':
					return new ItemResult(Double.valueOf(s), index);
				case 'S':
					return new ItemResult(Short.valueOf(s), index);
				case 'B':
					return new ItemResult(Byte.valueOf(s), index);
				case 'C':
					return new ItemResult(Character.valueOf((char) Integer.parseInt(s)), index);
				default:
					return new ItemResult(null, index);
				}
			} catch (NumberFormatException e) {
	...
	protected ItemResult deserializeBytesArrayItem(byte[] bytes, int index, int end, List<SimpleSerializable> ssObjs, Type extraType) {
		...
		if (c2 == 'A' || c2 == 'Z' || c2 == 'Y') {
			...
			try {
				if (l2 < 0 && l2 != -2) {
					return new ItemResult(null, index); // throw error?
				} else {
					if (l2 == -2) {
						char c4 = (char) bytes[index++];
						int l3 = c4 - baseChar;
			...
			try {
				...
				case 'E':
					if (s != null && s.length() > 0) {
						Object eo = null;
						Class<?> enumType = exactGenericClass(extraType);
						if (enumType != null) {
							Enum<?>[] enumConstants = (Enum<?> []) enumType.getEnumConstants();
							for (int i = 0; i < enumConstants.length; i++) {
								if (enumConstants[i].name().equals(s)) {
			...
			try {
				if (clazz == int.class) {
					...
					if (clazz == int.class) {
						...
						int size = list.size();
						Class<?> enumType = field.getType().getComponentType();
						Enum<?>[] enums = (Enum<?>[]) Array.newInstance(enumType, size);
						for (int i = 0; i < size; i++) {
							String s = (String) list.get(i);
							if (s == null || s.length() == 0) continue;
							Enum<?>[] enumConstants = (Enum<?> []) enumType.getEnumConstants();
							for (int j = 0; j < enumConstants.length; j++) {
								if (enumConstants[j].name().equals(s)) {
									enums[i] = enumConstants[j];
									break;
								}
							}
					...
					if (o != null && o instanceof String && ((String) o).length() > 0) {
						...
						for (int i = 0; i < enumConstants.length; i++) {
							if (enumConstants[i].name().equals(o)) {
								eo = enumConstants[i];
								break;
							}
						}
